...
Current state, and the log of every non-obvious finding from building it. Contracts are in `SPEC.md`; verification approach is in `TESTING.md`.

**Build complete.** Every phase below is done. The only two things left are external and awaiting explicit go-ahead: publishing `getnestdoc` to npm (and reserving `nest-doc` as a placeholder), and opening an issue on `nestjs/docs.nestjs.com` describing the tool. Packaging, a real global install from the packed tarball, and licensing are all verified — see the Settled table below for what that surfaced.
...
| 9 | Release | packaging/install/licensing done; publish + docs.nestjs.com issue await go-ahead |
...
|---|---|---|
| Do published `.d.ts` retain JSDoc? | Yes — 220 `@publicApi` tags in `@nestjs/common@12.0.1` | ADR-0001 |
...
| Does Nest bootstrap fit the budget? | Yes, bundled: 97 ms of 150 ms | ADR-0005 |
...
| Exit code for "package ships no types"? | 3, not 1 — ARCHITECTURE §10's failure-modes table said exit 1, contradicting SPEC.md §5's own exit code table. Corrected | ARCHITECTURE §10, Phase 5 |
...
| Extraction result: 206/206 on first full run | Exactly 206 exports, 177 `isPublicApi`, `Injectable` preserves `InjectableOptions` and carries exactly 3 `@see` links — passed without needing a fix-up pass, once the filter-propagation trap above was handled correctly from the start | Phase 6 |
